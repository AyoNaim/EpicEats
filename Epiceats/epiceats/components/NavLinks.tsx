...
const NavLinks = ({text, type, icon, className}: LinkProps) => {
  return (
    <Button className={`${className} group ${type === 'food' ? 'rounded-3xl' : type === 'contact' ? 'rounded-3xl' : '' } group-hover:animate-all duration-500 flex justify-between items-center gap-2 border border-black bg-red-500`}>
